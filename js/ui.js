...
export function createUI(eventBus, gameService, rootEl) {
    ...
    function renderBoard(cards) {
      ...
      cards.forEach(card => fragment.appendChild(buildCardElement(card)));
    ...
    function markCardsMatched(firstId, secondId) {
      ...
      const a = els.board.querySelector(`[data-card-id="${firstId}"]`);
      const b = els.board.querySelector(`[data-card-id="${secondId}"]`);
      if (a) a.classList.add('is-matched');
      if (b) b.classList.add('is-matched');
    }
    ...
    function flipCardsFaceDown(firstId, secondId) {
      ...
      const a = els.board.querySelector(`[data-card-id="${firstId}"]`);
      const b = els.board.querySelector(`[data-card-id="${secondId}"]`);
      if (a) a.classList.remove('is-flipped');
      if (b) b.classList.remove('is-flipped');
    }
